...
class OllamaTranslator:
    ...
    def __init__(
        ...
        # 통계 추적
        self.stats = {"cache_hits": 0, "cache_misses": 0, "total_translations": 0}
...
위 텍스트의 장르를 하나만 선택하여 답해주세요 (단어만): """
            ...
            response = self.client.chat(
                ...
                options={"temperature": 0.3, "top_p": 0.8, "num_gpu_layers": self.num_gpu_layers},
            )
    ...
    def ensure_model_loaded(self) -> bool:
        ...
        try:
            ...
            test_response = self.client.generate(
                model=self.model_name, prompt="Hello", options={"num_predict": 1}  # 1토큰만 생성
            )
            return True
        except Exception as e:
    ...
    def _get_fallback_options(self, attempt: int) -> dict:
        ...
        fallback_configs = [
            # 기본 설정
            {"temperature": self.temperature, "top_p": 0.8, "top_k": 30, "repeat_penalty": 1.2, "seed": 42},
            # 더 보수적인 설정
            {"temperature": 0.05, "top_p": 0.6, "top_k": 20, "repeat_penalty": 1.3, "seed": 123},
            # 다른 시드와 매개변수
            {"temperature": 0.2, "top_p": 0.7, "top_k": 25, "repeat_penalty": 1.1, "seed": 456},
            # 최대한 보수적
            {"temperature": 0.01, "top_p": 0.5, "top_k": 15, "repeat_penalty": 1.4, "seed": 789},
            # 다른 접근법
            {"temperature": 0.15, "top_p": 0.9, "top_k": 35, "repeat_penalty": 1.0, "seed": 999},
        ]
...
한국어 번역 (반드시 문어체로):"""
        ...
        for attempt in range(max_attempts):
            try:
                ...
                print(
                    f"   시도 {attempt + 1}: temp={options['temperature']}, top_p={options['top_p']}, seed={options['seed']}"
                )

                # Ollama Python 클라이언트로 번역 요청
                response = self.client.generate(model=self.model_name, prompt=simple_prompt, options=options)
                ...
                if translation:
                    ...
                    # 컨텍스트 일관성 적용
                    if self.context_manager:
                        validated_translation = self.context_manager.apply_context_corrections(
                            text, validated_translation
                        )
    ...
    def translate_chunks(self, input_dir: str, output_dir: str) -> Dict[str, any]:
        ...
        for chunk_info in chunk_index["chunks"]:
            chunk_file, success = self._translate_chunk_worker(
                chunk_info, chunks_dir, translated_chunks_dir, progress, progress_file, pbar
            )
            if success:
    ...
    def fix_translated_chunks(self, translated_dir: str) -> Dict[str, any]:
        ...
        for ko_file in pbar:
            try:
                ...
                if problems:
                    stats["problem_files"].append({"file": ko_file.name, "problems": problems})
                    ...
                    possible_paths = [
                        ...
                        Path(translated_dir).parent
                        / original_file.replace("_", "/").replace(".txt", "")
                        / "chunks"
                        / original_file,
                    ]
...
def main():
    ...
    parser.add_argument(
        "--genre", default="fantasy", choices=["fantasy", "sci-fi", "romance", "mystery", "general"], help="소설 장르"
    )
