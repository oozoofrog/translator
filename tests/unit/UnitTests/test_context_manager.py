...
def test_update_context():
    with tempfile.TemporaryDirectory() as tmp_dir:
        manager = TranslationContextManager("테스트책", tmp_dir)
        analysis_result = {
            "characters": [{"name": "Raistlin", "description": "마법사"}],
            "places": [{"name": "Shoikan Grove", "description": "숲"}],
            "terms": [{"name": "Soulforge", "category": "마법", "description": "마법 도구"}],
            "relationships": [{"character1": "Raistlin", "character2": "Caramon", "relationship": "형제"}],
        }
        ...
        assert {"character1": "Raistlin", "character2": "Caramon", "relationship": "형제"} in manager.context[
            "relationships"
        ]
        # 저장 후 재로드
        manager.save_context()
